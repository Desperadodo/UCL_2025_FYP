...
class DatasetAttr:
    ...
    # basic configs
    load_from: Literal["hf_hub", "ms_hub", "om_hub", "script", "file", "custom"]
    dataset_name: str
...
def get_dataset_list(dataset_names: Optional[list[str]], dataset_dir: str) -> list["DatasetAttr"]:
    ...
    for name in dataset_names:
        if isinstance(name, dict):
            loader = name.get("loader", "file")
            split  = name.get("split", "train")
            ds_name = name.get("dataset_name") or name.get("file_name") or ""

            if loader in ("unpuzzle", "postgres"):
                load_from = "custom"
            elif loader == "file":
                load_from = "file"
            else:
            ...
            dataset_attr = DatasetAttr(load_from, dataset_name=ds_name, split=split, loader=loader)
            dataset_attr.join(name)
        ...
        elif "loader" in dataset_info[name]:
            dataset_attr = DatasetAttr("custom", dataset_name=name, loader=dataset_info[name]["loader"])
