...
class MMPluginMixin:
    ...
    def _regularize_images(self, images: list["ImageInput"], **kwargs) -> dict[str, list["ImageObject"]]:
        ...
        results = []
        for image in images:
            # Check if this is an H5 file - if so, return None to avoid PIL processing
            if isinstance(image, str) and image.endswith('.h5'):
                # Return None for H5 files to avoid expand_mm_tokens generating multiple <image> tokens
                # The H5DynamicCollator will handle the actual H5 file loading
                results.append(None)
                continue
    ...
    def _get_mm_inputs(
        ...
        if len(images) != 0:
            ...
            # Filter out None values (H5 files) before passing to image_processor
            valid_images = [img for img in images if img is not None]
            
            if valid_images:  # Only process if there are valid images
            ...
            if getattr(processor, "image_do_pan_and_scan", False):  # gemma3 image processor
                ...
                mm_inputs.update(image_processor(valid_images, return_tensors="pt", **image_processor_kwargs))
    ...
    def process_messages(
        ...
        if self.expand_mm_tokens:
            ...
            if "pixel_values" in mm_inputs:
                ...
                # No pixel_values (e.g., all images are H5 files) - use single token
                image_seqlen = 1
        else:
            image_seqlen = 1
