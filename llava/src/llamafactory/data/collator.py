...
class MultiModalDataCollatorForSeq2Seq(DataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        ...
        fake_input_ids = []
        # Check for the custom flag before adding fake inputs
        is_h5_processed = any(feature.get("is_h5_processed", False) for feature in features)
        
        if (
            not is_h5_processed and self.template.mm_plugin.image_token is not None and sum(batch_imglens) == 0 and sum(batch_vidlens) == 0
        ...
        if (
            not is_h5_processed and self.template.mm_plugin.audio_token is not None and sum(batch_audlens) == 0
        ):  # avoid process hanging in zero3/fsdp case
...
class SFTDataCollatorWith4DAttentionMask(MultiModalDataCollatorForSeq2Seq):
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, "torch.Tensor"]:
        # --- PRE-CALL DEBUG ---
        if features:
            first_feature_before = features[0]
            if "input_ids" in first_feature_before:
                # Safe way: directly use the known token string, as self.template can be None in workers
                image_token_id = self.tokenizer.convert_tokens_to_ids("<image>")
                
                count_before = first_feature_before["input_ids"].count(image_token_id)
                """print(f"--- COLLATOR DEBUG: BEFORE super().__call__ ---")
                print(f"Image token count in first sample: {count_before}")
                print(f"First 50 input_ids: {first_feature_before['input_ids'][:50]}")
                print("-------------------------------------------------")"""

        original_features = super().__call__(features)

        # --- POST-CALL DEBUG ---
        if "input_ids" in original_features and original_features["input_ids"].nelement() > 0:
            first_sequence_after = original_features["input_ids"][0]
            # Safe way: directly use the known token string
            image_token_id = self.tokenizer.convert_tokens_to_ids("<image>")

            count_after = (first_sequence_after == image_token_id).sum().item()
            """print(f"--- COLLATOR DEBUG: AFTER super().__call__ ---")
            print(f"Image token count in first sample: {count_after}")
            print(f"First 50 input_ids: {first_sequence_after[:50].tolist()}")
            print("------------------------------------------------")"""
        
        if self.block_diag_attn and self.attn_implementation != "flash_attention_2":
            original_features["attention_mask"] = prepare_4d_attention_mask(original_features["attention_mask"], self.compute_dtype)

        for key, value in original_features.items():  # cast data dtype for paligemma
            if torch.is_tensor(value) and torch.is_floating_point(value):
                original_features[key] = value.to(self.compute_dtype)
