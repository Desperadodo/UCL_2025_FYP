...
register_template(
    ...
    format_tools=ToolFormatter(tool_format="llama3"),
    format_prefix=EmptyFormatter(slots=[{"bos_token"}]),
    default_system="You are an expert pathology AI. Your sole task is to analyze the provided histopathology image. Base your answer exclusively on visual evidence from the slide. Justify all conclusions by describing the specific morphological and architectural features you observe.",
    stop_words=["<|eot_id|>", "<|eom_id|>"],
    replace_eos=True,
    mm_plugin=get_mm_plugin(name="llava", image_token="<image>"),
)
