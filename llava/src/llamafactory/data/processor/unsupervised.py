...
class UnsupervisedDatasetProcessor(DatasetProcessor):
    def _encode_data_example(
        ...
        # --- START: ALIGNED IMAGE TOKEN EXPANSION FOR INFERENCE ---
        # This is the correct place for inference, as the single <image> placeholder
        # has just been added by `process_messages` above.
        image_seq_len = getattr(self.data_args, "image_seq_length", 1536)
        if image_seq_len >= 0 and images:
            ...
            # Find the user turn and expand the image placeholder.
            for i in range(len(messages)):
                if messages[i]["role"] == "user" and image_placeholder in messages[i]["content"]:
                    original_content = messages[i]["content"]
                    # Replace only the first occurrence to be safe.
                    expanded_content = original_content.replace(image_placeholder, image_placeholder * effective_len, 1)
                    messages[i]["content"] = expanded_content
                    break # Assume only one turn contains images
        # --- END: ALIGNED IMAGE TOKEN EXPANSION FOR INFERENCE ---
