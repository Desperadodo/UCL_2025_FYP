...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        help="Split ratio for training and validation data. Default: 0.8"
    )
    ...
    parser.add_argument(
        "--dataset-type",
        choices=["webdataset", "csv", "synthetic", "auto", "multimodal", "postgres", "unpuzzle"],
        default="auto",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help='Use multi-positive contrastive loss, requires passing `case_ids` to loss function.'
    )
    parser.add_argument(
        "--loss-dist-impl",
        default=None,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to folder containing images for multimodal dataset."
    )

    # arguments for WSI dataset
    parser.add_argument(
        "--wsi_base_path", 
        type=str, 
        default=None,
        help="Base path containing WSI feature files"
    )
    parser.add_argument(
        "--patches_per_wsi", 
        type=int, 
        default=1024,
        help="Number of patches to sample per WSI during training"
    )
    parser.add_argument(
        "--feature_suffix", 
        type=str, 
        default=".h5",
        help="File extension of feature files"
    )

    args = parser.parse_args(args)

    # Handle custom model configurations
    if args.model.lower() in CUSTOM_MODEL_CONFIGS:
        config = CUSTOM_MODEL_CONFIGS[args.model.lower()]
        
        # Set model configuration
        args.custom_config = config
        
        # Set default parameters based on model type
        default_params = get_default_params("vit")  # Use ViT defaults for custom models
        for name, val in default_params.items():
            if getattr(args, name) is None:
                setattr(args, name, val)
                
        # Force custom text for our models
        args.force_custom_text = True
        
        # Set model-specific parameters
        if config["model_type"] == "coca":
            args.coca_mode = True
            
    elif 'timm' not in args.opt:
        # Default parameter handling for standard models
