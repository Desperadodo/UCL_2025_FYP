...
from .model import CLIP, CustomCLIP, CustomTextCLIP, WSI_CLIP, convert_weights_to_lp, convert_to_custom_text_state_dict,\
    resize_pos_embed, get_cast_dtype, resize_text_pos_embed, set_model_preprocess_cfg
from .coca_model import CoCa
from .loss import ClipLoss, DistillClipLoss, CoCaLoss, SigLipLoss, WSIClipLoss, MultiPosClipLoss
...
def _rescan_model_configs():
    # this function searches all the json files in the model_configs directory and put them into the _MODEL_CONFIGS dict
    # the key is the name of the model, and the value is the config dict
...
def create_model(
    ...
    Examples:
        ...
        model = create_model('ViTh')

        # Create WSI model
        model = create_model('ViT-B/32-wsi-MIL')
        or
        model = create_model('ViT-B/32-wsi-Gigapath')
    ...
    # load model cfg from local config file or hf hub if not passed in
    model_cfg = model_cfg or get_model_config(model_name)

    # wsi model MI-Zero and Gigapath style
    is_wsi_model = False
    if model_cfg is not None and isinstance(model_cfg, dict):
        is_wsi_model = model_cfg.pop("is_wsi_model", False)
    ...
    custom_vision = model_cfg.pop('custom_vision', False)  # Remove custom_vision flag

    model_cfg = dict(model_cfg, **model_kwargs)  # merge cfg dict w/ kwargs (kwargs overrides cfg)
    if is_wsi_model:
        model = WSI_CLIP(**model_cfg, cast_dtype=cast_dtype)
        model.is_wsi_model = True
    elif custom_text and custom_vision:
...
def create_loss(args):
    ...
    elif args.multi_positive_loss:
        return MultiPosClipLoss(
            local_loss=args.local_loss,
            gather_with_grad=args.gather_with_grad,
            cache_labels=True,
            rank=args.rank,
